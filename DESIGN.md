...
- 크롬은 sticky 헤더 하나(`h-14`)뿐 — 하단 탭바도, 사이드 레일도, 푸터도 없다(web 한정 — admin은 좌측 사이드바를 쓴다, §5 Navigation)
