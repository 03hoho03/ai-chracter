...
### Navigation
- **크롬은 sticky 헤더 하나뿐이다.** `sticky top-0 z-30 h-14 border-b border-border bg-background`, 내부는 `mx-auto max-w-5xl px-4 sm:px-6`. **`max-w-6xl`이 아니라 `max-w-5xl`인 이유**: 헤더 폭 자체엔 근거가 없었지만 본문 max-width엔 있다(§Layout containers, `:327`의 잉크 폭 실측) — 근거 있는 쪽에 맞춰 가장 넓은 본문(홈·즐겨찾기·`/my`의 `max-w-5xl`)과 정렬했다. 1440px 실측: 헤더 로고 left와 `<main>` 콘텐츠 박스 left가 둘 다 224.5px, drift 0.00. 하단 탭바·사이드 레일·푸터는 **존재하지 않으며, 추가하지 않는다** — 크롬은 얇고 항상 동일해야 한다.
- **이 금지의 대상은 전역 크롬이다 — `<main>` 안에서 한 라우트의 콘텐츠를 여러 열로 나누는 것은 이 Don't의 대상이 아니다**(`BuilderLayout`의 lg 2단, `/studio/images`의 lg 3단). 판별 기준은 폭이 아니라 셋이다 — ① 모든 라우트에 상시 존재하는가 ② 역할이 **화면 전환**인가 ③ `h-14` 밖에 크롬 줄을 더하는가. 셋 중 하나라도 참이면 사이드 레일이고, 전부 거짓이면 페이지 콘텐츠다. 이 레이아웃은 사이드 레일이 아니다 — 그 라우트에만 있어 상시 존재하지 않고(①거짓), 열 사이 전환은 화면 전환이 아니라 같은 화면 안의 배치이며(②거짓), 크롬은 여전히 위 `h-14` 헤더 하나뿐이다(③거짓). `/studio/images`의 좌열(보관함)·우열(생성 옵션)도 같은 세 축으로 사이드 레일이 아니다 — 그 라우트에만 있고, lg 미만에서는 탭 스트립의 아이콘 버튼으로 여는 바텀시트로 접힌다(화면 전환이 아니라 같은 화면의 다른 표현). 아래 채팅 더보기 패널도 같은 세 축으로 사이드 레일이 아니라고 판정된 선례다. **이 예외는 `<main>` 내부의 다열 레이아웃에 한정되며 전역 셸로 번지지 않는다** — 새 전역 사이드바나 상시 내비게이션 레일을 추가할 근거로 쓰지 말 것. 크롬은 여전히 `h-14` 헤더 하나뿐이다.
