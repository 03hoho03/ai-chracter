...
- **이 금지의 대상은 전역 크롬이다 — `<main>` 안에서 한 라우트의 콘텐츠를 여러 열로 나누는 것은 이 Don't의 대상이 아니다**(`BuilderLayout`의 lg 2단). 판별 기준은 폭이 아니라 셋이다 — ① 모든 라우트에 상시 존재하는가 ② 역할이 **화면 전환**인가 ③ `h-14` 밖에 크롬 줄을 더하는가. 셋 중 하나라도 참이면 사이드 레일이고, 전부 거짓이면 페이지 콘텐츠다. 이 레이아웃은 사이드 레일이 아니다 — 그 라우트에만 있어 상시 존재하지 않고(①거짓), 열 사이 전환은 화면 전환이 아니라 같은 화면 안의 배치이며(②거짓), 크롬은 여전히 위 `h-14` 헤더 하나뿐이다(③거짓). 아래 채팅 더보기 패널도 같은 세 축으로 사이드 레일이 아니라고 판정된 선례다. **이 예외는 `<main>` 내부의 다열 레이아웃에 한정되며 전역 셸로 번지지 않는다** — 새 전역 사이드바나 상시 내비게이션 레일을 추가할 근거로 쓰지 말 것. 크롬은 여전히 `h-14` 헤더 하나뿐이다.
