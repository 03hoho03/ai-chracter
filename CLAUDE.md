...
- **장식·전환 애니메이션에는 `motion-safe:`를 붙인다.** 어두운 방에서 갑작스러운 움직임은 놀람이다. **예외는 진행 표시다** — 로딩 스피너(`animate-spin`)·스켈레톤(`animate-pulse`)은 멈추면 "멈춘 UI"로 읽히므로 가드하지 않는다(근거: `DESIGN.md` §5 Motion).
