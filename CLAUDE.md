...
- **`primary`는 hue 강조가 아니라 `foreground`와 같은 값**(최고 대비 무채색 채움)이다. 채움 위 텍스트는 `primary-foreground`로 뒤집는다 — `destructive`도 동일한 반전 쌍을 유지해야 한다(밝힌 빨강 위 흰 텍스트는 3.68:1로 AA 미달).
- **UI는 무채색**이다. 색은 사용자 콘텐츠(썸네일, 스탯 스와치)와 `destructive`에만 존재한다. `destructive`는 항상 `/10` 틴트이며 솔리드 레드 채움은 이 시스템에 없다.
