...
- 코드 주석·docstring·테스트 이름과 추적되는 문서는 그 자리에서 뜻이 서야 한다. `tasks/` 문서, 결정 번호(`CL-12`·`D-5`), 개인 FE 플러그인 규칙 코드(`FSD-06`·`TS-01`)를 인용하지 말고 이유를 문장으로 쓴다. 결정 추적은 커밋 메시지·PR 본문에 남긴다(문장이 먼저, 번호는 곁들일 때만). 추적되는 저장소 문서는 파일명 + 절 제목으로 가리킨다(`§n` 금지). 미래 작업은 문서 대신 할 일 자체를 적는다. CI `citations` 가 추가된 줄을 검사하고, 진짜 코드 식별자(응답에 나가는 규칙 이름 등)는 같은 줄에 `cite-ok` 와 이유를 적는다.
