...
class CloverLedger(Base):
    ...
    **append-only다 — UPDATE·DELETE를 하지 않는다.** 보존은 무한 누적이고 삭제 배치를
    ...
    # Literal이 강제 지점이다(clover-techspec.md §2-2의 표 8종 +
    # clover-page-goal-prompt.md CE-10의 `mission_grant`·`expire_burn`).
...
class CloverLot(Base):
    """clover-page-goal-prompt.md CE-3. 지급 1건 = 로트 1행. `remaining`이 소진 순서
    (CE-5, 만료 임박 우선)대로 줄어드는 잔여량이고, `users.clover_balance`는 이 테이블
    `remaining` 합의 캐시다(CE-4 불변식: `SUM(remaining) == clover_balance`).
    ...
    `clover_ledger`를 지금 맞춰 바꾸지 않는 이유는 이번 런의 범위 밖이라서다(CE-3).
    ...
    # NULL = 무기한(어드민 수동 지급·환불, CE-11). 값이 있으면 그 시각(KST 자정 기준,
    # CE-7)에 소멸 대상이다 — 진실은 만료 배치(CE-9)이고 이 컬럼은 배치가 아직 안 돈
    # 구간에서만 "만료 예정"을 뜻한다(CE-8).
    ...
    #
    # 인덱스는 goal-prompt CE-3 원문이 "2개"(`remaining > 0` 부분 인덱스,
    # `(user_id, expires_at, created_at)`)로 적었지만 하나로 겸한다 — 두 조회(CE-5 소진:
    # `ORDER BY expires_at ASC NULLS LAST, created_at ASC` / CE-34 회수:
    # `ORDER BY created_at DESC`)가 **행을 고르는 조건**(`user_id = :u AND remaining > 0`)을
    # 공유하고, 그 조건이 곧 이 부분 인덱스이기 때문이다. 즉 인덱스가 먹는 것은 조회 범위이지
    # 정렬이 아니다.
    #
    # 🔴 CE-34의 `created_at DESC`는 이 인덱스로 정렬이 해결되지 **않는다** — B-tree 역순
    # 스캔은 `expires_at DESC, created_at DESC`를 내지 순수 `created_at DESC`가 아니고,
    # 로트마다 `expires_at`이 섞이는 것이 정상이다(무기한 NULL + 만료일 여럿). 플래너가
    # 범위를 좁힌 뒤 Sort로 보정하며, 유저당 활성 로트가 10행 미만이라(CE-6) 그 정렬 비용이
