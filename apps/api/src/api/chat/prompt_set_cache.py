...
# prompt-scope-techspec.md §3-4 (PS-15). 레인마다 키가 하나다 — `prompt_set:active:story`
# 등. PS-7: 옛 키(`prompt_set:active`)를 재사용하면 옛 코드가 쓴 48행 통짜 세트를 새 코드가
...
async def get_cached_active_prompt_set(lane: PromptLane) -> tuple[PromptSet, list[PromptSection]] | None:
    ...
    세트가 없다는 사실 자체는 절대 캐싱하지 않는다(negative caching 금지, §8-1) —
...
async def set_cached_active_prompt_set(
    ...
    호출이 누락되는 경로가 생겼을 때만 옛 문안을 걷어내는 안전망이다(§8-1).
...
async def invalidate_active_prompt_set(lane: PromptLane) -> None:
    """`lane` 키 하나만 지운다(PS-15) — 게시는 레인 단위 이벤트이고, 셋을 다 지우면 다른
    레인의 캐시를 이유 없이 버린다.
