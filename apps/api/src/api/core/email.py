...
# apps/api/CLAUDE.md: 외부 HTTP 호출은 Depends로 감싸 테스트에서
...
async def send_email_resend(to: str, subject: str, body: str) -> None:
    ...
    KB짜리 JSON 왕복이라 오래 걸릴 이유가 없다 — 응답 이후 백그라운드에서 도는 호출이
...
def get_email_sender() -> EmailSender:
    """FastAPI dependency: settings.email_provider로 분기한다. 프로바이더가 2개뿐이라
