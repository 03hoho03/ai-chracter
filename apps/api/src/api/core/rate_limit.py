...
연속 충전한다. 요청 수만큼 한 번에 차감하고, 모자라면 부분 차감
...
키 프리픽스는 두 기구 모두 `rate_limit:`이다. 테스트
...
# `ZoneInfo("Asia/Seoul")`이 아니라 고정 오프셋이다. KST는 DST가
...
def seconds_until_kst_midnight(now: datetime) -> int:
    """tz-aware `now`(UTC든 KST든)에서 다음 KST 자정까지 남은 초.
    ...
    (=KST 09:00)에서 끊기는데 예외도 로그도 없어 KST 자정 기준 일일 창이 조용히 깨진다.
...
async def check_rate_limit(
    ...
    `window_seconds`의 기본값 1시간은 발송 엔드포인트 5개 호출부가 그대로 쓴다 — 바꾸면
    그 5곳의 창이 같이 바뀐다. 일일 창은 호출자가 `seconds_until_kst_midnight()`를 넘긴다.

    ⚠️ 호출자는 이 함수를 DB 조회보다 먼저 불러야 한다 — 계정 존재
...
async def refund_tokens(
    ...
    *,
    ...
    """차감했던 토큰 `count`개를 용량 천장까지 돌려놓는다 —
