"""자가호스팅 Bugsink에는 서버측 스크러빙이 없다 —
SDK 옵션이 유일한 방어선이다. 실제 `sentry_sdk.init()` 호출과 DSN 등 배포별 설정은
`main.py`/`core/config.py`에 있다. 이 모듈의 옵션 빌더는 `init()`을 직접 부르지 않는다 — 그래야
테스트가 전역 SDK 상태를 건드리지 않고 `sentry_sdk.Client(**build_sentry_options())`로
프로덕션과 같은 옵션을 재현해 검증할 수 있다(옵션이 테스트와 프로덕션에서 갈리면 테스트가
아무것도 보증하지 못한다).

`capture_dependency_failure`는 흡수된 장애(`logger.warning`)를 Bugsink 이벤트로도
승격하는 공통 지점이다 — 승격 대상 13곳이 같은 두 줄(로그는 유지 + 호출 한 줄)을 반복해서
헬퍼로 뺐다. 전역 `sentry_sdk.capture_exception`을 그대로 호출하므로 `init()`을 부르지 않고,
DSN이 비어 `init()`이 안 불린 환경(dev·테스트)에서는 활성 클라이언트가 없어 no-op이다.
"""
...
def build_sentry_options() -> dict[str, Any]:
    """`sentry_sdk.init(**build_sentry_options())` / 테스트의 `sentry_sdk.Client(**...)`가
    함께 쓰는 스크러빙 옵션.

    - `max_request_body_size="never"`: JSON 요청 본문(채팅 메시지 원문)은 `send_default_pii`와
      무관하게 첨부된다 — 크기 게이트만 탄다. `"never"`가 그 게이트를 항상 닫는 유일한
      값이다.
    - `include_local_variables=False`: 채팅 프롬프트·히스토리·비밀번호 재설정 링크·인증 코드가
      스택 프레임 지역변수로 산다 — 임의 문자열이라 키 이름 매칭 스크럽으로는 원리적으로
      못 잡는다.
