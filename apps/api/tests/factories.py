...
# `POST /dev/session-echo` 대신 세션을 직접 만들어 쿠키에 넣는다. 넣는 값
# `{"user_id": str(...)}`은 프로덕션 로그인 경로 4곳(`auth/router.py:258,327,372,413`)이
# 세션에 담는 것과 글자까지 같다 — 그래서 이 헬퍼로 선 세션은 실제 로그인 세션과 구분되지 않는다.
