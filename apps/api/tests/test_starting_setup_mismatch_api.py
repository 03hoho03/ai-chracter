"""시작설정 불일치 상태(방의 `starting_setup_entity_id`는
non-null인데, 방이 고정한 `content_version_id` 아래 그 `entity_id`를 가진 `StartingSetup`
행이 없음)가 SSE 제너레이터·비스트리밍 라우트를 뚫는 것을 실측으로 확인하는
재현 테스트다.

처음에는 이 파일이 수정(상류 정규화) 이전의 "빨강"을 실측으로 남기는 것이 목적이었다
(apps/api/CLAUDE.md §테스트 정책 — 구현 뒤에 쓴 테스트는 초록이어도 신호가 없다. 소스
0줄 변경 상태에서 작성됐다). 그 수정으로 `_require_starting_setup`이 들어오면서 재현 5개는
더 이상 예외가 새지 않고 400 응답으로 정규화되므로, 그 5개의 단언을 `pytest.raises(...)`에서
400 응답 단언으로 재작성했다 — 판정은 같은 픽스처에서 처방 전에는 `ExceptionGroup`/`AssertionError`가
...
prompt-render 결함 계열의 전용 파일이라 결함 계열이 다른 이 재현을 섞지 않기 위해서다.

`chat_rooms.starting_setup_entity_id`에는 FK가 없어(migrations/versions/
...
async def _make_mismatched_story_room(
    ...
    """시작설정 불일치 재현 상태를 만든다 — 스토리 방을 정상 생성한 뒤(오프닝 메시지까지 정상
...
async def _still_alive(db_session: AsyncSession) -> int | None:
    """스모크 체크 — "실패가 이 요청 안에 갇혔다"는 신호로만 쓴다(항진명제라
    처방 전후 모두 초록일 것으로 예상됨). 판정 근거로 쓰지 않는다."""
    return cast(int | None, await db_session.scalar(sa.select(sa.func.count()).select_from(Content)))


# `_require_starting_setup`이 던지는 400의 `detail` 문자열.
...
async def test_send_message_on_mismatched_story_room_returns_400(
    ...
    (본문이 시작되기 전이라는 증거), 사용자 메시지도 커밋되지 않아야 한다."""
    user = _make_user()
    db_session.add(user)
    await db_session.flush()
    ...
    # `Depends`에서 죽어 본문이 시작되지 않으므로 사용자 메시지가 커밋되지
...
async def test_regenerate_message_on_mismatched_story_room_returns_400(
    ...
    # 한 턴을 직접 추가한다(적대적 리뷰 발견 — 이전엔 ASSISTANT만 이어붙여 USER 없이
...
async def test_get_play_guide_on_mismatched_story_room_returns_400(
    ...
    """비스트리밍 2곳 중 하나 — `get_play_guide`가 이제
    `_require_starting_setup`을 불러 불일치를 400으로 정규화한다(수정 전에는
...
async def test_reset_chat_room_on_mismatched_story_room_returns_400(
    ...
    """비스트리밍 2곳 중 하나 — `reset_chat_room`이 이제
    `_require_starting_setup`에서 400을 받는다(수정 전에는 `_insert_opening_
    message`의 `assert`가 맨 `AssertionError`를 던졌다). 그 자리는 메시지 삭제·turn_count
    대입 뒤지만 `db.commit()` 전이라 데이터는 안전하다."""
    user = _make_user()
    db_session.add(user)
    await db_session.flush()
    genre = await _get_genre(db_session)
...
async def test_get_chat_room_on_mismatched_story_room_stays_a_degraded_200(
    ...
    """회귀 가드. `_to_response`는 불일치 400 정규화가 **의도적으로 제외**한
    넷째 호출부다. 수정 전에도, 수정 이후에도 이 라우트는
    크래시 없이 `contentType="story"`인데 `stats`·`contentSnapshot`·`startingSetupId`가
    전부 null인 200을 돌려줘야 한다 — 이게 400으로 바뀌면 처방이 `_to_response`
    까지 덮은 것이고, `pin_latest_version`이 방을 잠그는 결과로 이어진다."""
    user = _make_user()
    db_session.add(user)
    await db_session.flush()
    genre = await _get_genre(db_session)
...
async def test_write_endpoints_stay_200_on_mismatched_story_room(
    ...
    """`_to_response`를 처방에서 뺀 이유 — `rename_chat_room`·`pin_latest_version`·
    `acknowledge_version_upgrade`는 셋 다 `_to_response`(`_resolve_starting_setup`을 그대로
    쓴다, 처방에서 의도적으로 제외됨)를 거치므로 불일치 방에서도 200을 유지해야
    한다. 여기가 400이 되면 `pin_latest_version`이 방을 잠근다. 특히
    `pin_latest_version` 호출 **뒤에** `GET /chat-rooms/{room_id}`가 여전히 200이어야
    한다(스토리 경로 `pin_latest_version` 테스트가 이 파일 이전엔 0건이었다).
...
async def test_character_room_send_message_and_play_guide_are_unaffected(
    ...
    """안전망 — `starting_setup_entity_id is None`인 정상 캐릭터 방은 이 결함 계열과
    무관하다(`_resolve_starting_setup`의 조기 반환이 캐릭터 챗의 정상 신호).
    처방이 이 경계를 잘못 잡으면 캐릭터 방 전체가 영향을 받으므로, 재현 테스트와 같은 파일에
    캐릭터 방이 **지금 정상 동작한다**는 기준선을 남겨 둔다. **`send_message`도 실제로
    호출한다**(SSE 3경로 중 `_starting_setup_dependency`를 거치는 경로) — 적대적 리뷰가
