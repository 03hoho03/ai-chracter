...
작성 당시에는 어떤 엔드포인트에도 아직 안 붙어 있었다(지금은 쓰기 라우트들에 붙어 있다).
그래서 HTTP 라운드트립으로는 테스트할 수 없었다 — 테스트 전용 임시 라우트를 새로 만드는 대신,
...
갈아끼운다"는 것이라, 당시 아무 라우트에도 안 걸려 있던 의존성 자체를 단위 테스트하는 이 상황에는
맞지 않는다.
"""
