"""프롬프트 DB 이관 게이트 — DB 시드에서 읽은 활성 세트를 렌더러로 조립한 결과가
이관 전 코드가 낸 골든 25개와 바이트 단위로 같은지 본다.
...
`prompt_sets`가 레인별로 나뉘어(story/character/publish_filter) `active_prompt_set` 픽스처는
`GOLDEN_CASES`의 각 항목이 들고 있는 레인을 `request.param`으로 받는 간접(indirect)
파라미터화 픽스처다. 활성 세트는 `load_active_prompt_set`으로 고른다 — **프로덕션이 실제로
고르는 규칙과 같은 것**이다. 레인마다 published가 여럿인 것이 정상이라(마이그레이션
