...
prompt-scope-techspec.md §2-2 (M1), prompt-scope-goal-prompt.md PS-9·PS-9a·PS-11. 프롬프트
레인 분리(story/character/publish_filter)의 첫 단계 — `prompt_sets`에 `lane` 컬럼을 붙이고
인덱스 3개를 레인 축으로 교체한다. 문안 이관(M2)은 후속 리비전이다.

**`lane`은 Postgres enum이 아니라 `Text`다**(PS-9) — `status`/`kind`/`channel`과 같은 관례고,
...
**`server_default`는 이 리비전 안에서만 쓰고 뗀다**(PS-9a) — `add_column`으로 기존 행을 전부
...
인덱스 3건 교체 — `legal_documents`(`5bef71fc8f50`)와 글자 그대로 같은 모양이다(PS-11):
...
`ix_prompt_sets_published_at`(`published_at DESC` 단독)은 건드리지 않는다 — 이 런 뒤에는 이
인덱스를 쓰는 실사용 쿼리가 남지 않지만(그 자리는 `ix_prompt_sets_lane_published_at`로 옮겨간다),
무관한 죽은 코드는 삭제하지 말고 언급하라는 규약 때문이다(PS-11). 삭제는 별건.
...
def upgrade() -> None:
    """Upgrade schema."""
    # PS-9a: server_default 로 기존 행을 전부 'legacy' 로 백필한 뒤 같은 리비전에서 뗀다.
