...
import { useViewImageGenerationsMutation, type AdminImageGenerationDetailListResponse } from "../api/useViewImageGenerationsMutation";

/** 카테고리 필수·사유 텍스트 trim 후 비어있으면 안 됨 — `pages/chat-messages/ui/ViewReasonDialog.tsx`
 * 동형(비면 서버로 요청이 나가기 전에 필드 에러로 잡는다, BE도 422로 막지만 FE에서 먼저). */
...
type ViewReasonDialogProps = {
  userId: string;
  onCancel: () => void;
  onConfirmed: (data: AdminImageGenerationDetailListResponse) => void;
};

/** 이 화면의 진입 게이트 다이얼로그 — image-monitoring-goal-prompt.md IM-2, `chat-messages/ui/
 * ViewReasonDialog.tsx` 동형. `__root.tsx`에 콜러블로 마운트하지 않고 페이지 안에 직접 둔다.
 * 사유를 라우터 state나 전역 콜러블로 넘기면 새로고침 시 사라져 빈 화면이 되지만, 이 컴포넌트는
 * `ImageGenerationViewPage`가 결과를 아직 못 받은 동안 항상 그 자리에서 다시 렌더되므로
 * 새로고침해도 다이얼로그가 다시 뜬다. */
export function ViewReasonDialog({ userId, onCancel, onConfirmed }: ViewReasonDialogProps) {
  const viewMutation = useViewImageGenerationsMutation(userId);
  const {
    control,
    register,
    handleSubmit,
  ...
  return (
    <Dialog open onOpenChange={(open) => !open && onCancel()}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle>이미지 열람 사유</DialogTitle>
          <DialogDescription className="break-keep">
            이 유저의 생성 이미지를 보려면 사유가 필요해요. 열람 기록이 남습니다.
          </DialogDescription>
        </DialogHeader>
...
/** 카테고리 존재는 스키마가 보장한 뒤에만 호출된다(`chat-messages`의 formToViewRequest와 동형). */
