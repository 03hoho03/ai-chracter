...
type Props = {
  kind: LegalKind;
  document: AdminLegalDocumentResponse | undefined;
  isPending: boolean;
  isError: boolean;
  draftBody: string;
...
export function LegalEditor({ kind, document, isPending, isError, draftBody, onDraftBodyChange }: Props) {
  ...
  return (
    <div className="flex flex-col gap-6">
      <section className="flex flex-col gap-1 rounded-xl border border-border bg-card p-6">
        ...
        {document.published ? (
          <p className="text-sm text-foreground">
            ...
            {" · "}
            {DATE_TIME_FORMATTER.format(new Date(document.published.publishedAt))} 게시
      ...
      <section className="flex flex-col gap-3">
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
          <Textarea
            ...
            className="max-h-[32rem] min-h-[32rem] resize-none overflow-y-auto"
          />
          <div className="max-h-[32rem] overflow-y-auto rounded-lg border border-border bg-card p-4">
            {draftBody.trim() ? (
      ...
      <section className="flex flex-col gap-3 rounded-xl border border-border bg-card p-6">
        ...
        {versionsQuery.isPending && <div className="h-32 animate-pulse rounded-lg bg-muted" />}
        {versionsQuery.isError && (
          <p className="text-sm text-destructive-text">게시 이력을 불러오지 못했어요.</p>
        )}
        {versionsQuery.data && (
          versionsQuery.data.items.length === 0 ? (
            <p className="text-sm text-muted-foreground">게시 이력이 없어요.</p>
          ) : (
                ...
                <TableBody>
                  {versionsQuery.data.items.map((item) => (
                    <TableRow key={item.version}>
                      ...
                      <TableCell>{DATE_TIME_FORMATTER.format(new Date(item.publishedAt))}</TableCell>
