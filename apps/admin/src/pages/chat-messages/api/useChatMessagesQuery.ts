...
/** 더보기(`GET .../messages`)는 로그를 쌓지 않는 순수 조회라 `useQuery`로 선언하지만,
 * `useInfiniteQuery`는 쓰지 않는다 — 첫 페이지는 이 쿼리가 아니라 별도의 POST 뮤테이션
 ...
 * 호출부가 await 후 정확히 한 번만 수행한다. */
export function useChatMessagesQuery(roomId: string) {
