...
export function ContentDetailPage({ contentId }: ContentDetailPageProps) {
  ...
  return (
    <main className="mx-auto flex max-w-3xl flex-col gap-6 px-6 py-10">
      ...
      {contentDetailQuery.isPending && <div className="h-64 animate-pulse rounded-xl bg-muted" />}

      {contentDetailQuery.isError && (
        <p className="text-sm text-destructive-text">작품 정보를 불러오지 못했어요. 잠시 후 다시 시도해주세요.</p>
      )}
            ...
            <div className="flex gap-4">
              ...
              <div className="flex min-w-0 flex-col justify-center gap-1">
                ...
                <p className="text-xs text-muted-foreground">
                  {DATE_TIME_FORMATTER.format(new Date(contentDetailQuery.data.createdAt))} 등록
                </p>
              </div>
            </div>
                ...
                <TableBody>
                  {contentDetailQuery.data.versions.map((version) => (
                    <TableRow key={version.id}>
                      ...
                      <TableCell>
                        {version.publishedAt ? DATE_TIME_FORMATTER.format(new Date(version.publishedAt)) : "-"}
                      </TableCell>
                      <TableCell>{DATE_TIME_FORMATTER.format(new Date(version.createdAt))}</TableCell>
                    </TableRow>
                  ))}
                </TableBody>
              </Table>
