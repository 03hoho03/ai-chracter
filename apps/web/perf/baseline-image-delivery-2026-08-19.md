...
최적화는 **머지 커밋 없이 직선으로 진입**했다 — `e789818..9c7afe0` = **15커밋, 머지 0건**(`git rev-list --merges --count e789818..9c7afe0` → 0). 아래는 그 로그이고, 커밋 제목 앞의 스토리 번호는 뺐다.

```
6fd4457 2026-08-18 20:06:26 feat: AssetPurpose에 content-thumbnail 추가
3854d08 2026-08-18 20:10:01 feat: 썸네일 생성 유틸과 키 파생 규칙 추가
028ae1c 2026-08-18 20:14:27 feat: 업로드 완료 시 서버가 실제 파일 크기를 검증
9a932b3 2026-08-18 20:18:17 feat: 사용자 업로드 완료 시 썸네일 생성
a51df5f 2026-08-18 20:22:23 feat: AI 생성 이미지에 썸네일 생성
cbedf97 2026-08-18 20:26:08 feat: 상황별 이미지 블러본에 썸네일 생성
f9ca3d0 2026-08-18 20:33:26 feat: 기존 자산 썸네일 백필 스크립트
6555f52 2026-08-18 20:38:05 feat: content/router.py의 목록 응답 6곳이 썸네일을 서명
05c1979 2026-08-18 20:42:04 feat: 스튜디오 갤러리와 이미지 보관함이 썸네일을 서명
79b8559 2026-08-19 00:45:01 feat: FE 이미지 리사이즈 유틸 추가
27720af 2026-08-19 00:48:23 feat: uploadAsset에 리사이즈와 용량 검증 배선
34a91b7 2026-08-19 00:59:23 feat: 업로드 진입점 3곳에 형식 제한과 purpose 전환 적용
b0c058c 2026-08-19 01:10:22 feat: lazy loading과 첫 화면 우선 로딩 적용
6ba314a 2026-08-19 01:34:03 feat: 채팅 말풍선 이미지 CLS 방지
9c7afe0 2026-08-19 11:56:07 fix: 채팅 이미지 웰을 3:4로 바꿔 세로 이미지 축소를 되돌린다
```
...
| CLS | 0.10 | 동일하거나 소폭 개선 (US-014는 채팅 화면이라 홈에는 영향 없음) | <!-- cite-ok: 원본 전사 -->
...
`layout-shift` 엔트리만 믿으면 안 된다 — 시프트가 뷰포트 **밖**에서 일어나면 값이 0으로 나온다(채팅 말풍선 CLS 첫 측정에서 구버전도 0이 나왔다). 확실한 방법은 둘이고 **둘 다 하는 게 제일 좋다**:

1. 문제 구간을 `scrollIntoView`로 뷰포트에 올린 뒤 관찰한다.
2. 아래에 마커 엘리먼트를 두고 `getBoundingClientRect().top + scrollY`를 rAF로 샘플링해 **직접 이동량을 본다**(채팅 말풍선 CLS 측정의 511px가 이렇게 나온 값이다).
