...
- **자산**: 업로드는 `shared/lib/asset/uploadAsset(file, purpose)` 재사용(presigned 발급 → S3 직접 PUT은 `fetch`(외부 절대 URL이라 `apiClient` 아님) → complete). presigned GET URL은 매 조회 새로 서명되므로 **캐시 금지**.
