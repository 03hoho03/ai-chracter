...
type MyWorkCardMenuProps = {
   ...
   * `제목 없는 초안`을 받아 `제목 없는 초안 더보기`가 그 수만큼 생긴다. 초안 지연 생성 이후
   * 무명 초안 복수 보유는 엣지가 아니라 정상 상태다 — 판별자는 폴백 문자열 쪽(`MyWorkCard`)에서
...
/** 목록에서 바로 손대는 카드 관리 메뉴. 항목은 `item.kind`로 완전히 갈린다.
 *
    ...
    <ContentCardActionMenu title={title}>
      ...
      {item.kind === "draft" ? (
        ...
        <>
          ...
          {/* 버릴 편집분이 **있을 때만** 항목이 존재한다. 구분선도 같이 사라져야 한다 —
              혼자 남으면 메뉴 끝에 아무것도 가르지 않는 선이 붙는다. */}
          {hasUnpublishedEdits(item) && (
            <>
              <DropdownMenuSeparator />

              {/* 요구사항의 문자는 `편집 취소`지만 기획 단계의 열린 질문이 "'방금 누른 걸 취소'로 읽힐
                  ...
                  말하게 하고, 메뉴 → 모달 제목 → 실행 버튼이 한 어휘로 이어지게 했다. 기획이 제안한
