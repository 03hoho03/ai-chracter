// image-refact-goal-prompt.md IR-4 — 보관함은 lg 이상 좌열/미만 바텀시트로 상시 노출되고 탭이 아니다
// (IR-5). "library"를 뺀다 — `?tab=library`를 가리키는 <Link>가 0건이었다(grep 재확인 완료).
// 뒤 둘(변형/인페인트)은 아직 disabled라 값만 먼저 채운다(IT-10).
