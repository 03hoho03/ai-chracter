 ...
 * `assertNever`가 다시 총망라(exhaustive)가 된다. `window`를 `"image"`로 좁혀서 그대로 남기는
 ...
 * `getChatRateLimit`과 같은 이유). `code` enum이 auth 2종으로 늘면서 `RateLimitDetail`을
...
export function getImageRateLimit(error: unknown): ImageRateLimit | undefined {
  ...
  // 🔴 이 허용 목록은 **컴파일이 잡아 주지 않는다.** 위 유니언(`:8`)을
  ...
  // 🔴 `CLOVER_CONFIRM_REQUIRED`는 **일부러 빼 둔다**. 같은
...
export function formatImageRateLimitMessage(detail: ImageRateLimit): string {
  switch (detail.code) {
    ...
    // 🔴 채팅 문구("자정에 무료 한도가 돌아와요")를 **재사용하면 안 된다.**
