...
export type BuilderPreviewProps<TFieldValues extends FieldValues> = {
  /** 활성 탭(`TABS[activeTab].preview`)에서 파생된 값 — Shell이 계산해 넘긴다(상세 프리뷰는
   * 없어 카드·대화 2종뿐이다). */
  kind: "card" | "chat";
 ...
 * 그만큼만 그린다 — 개수를 채우는 건 이 위젯의 책임이 아니다(배경은 맥락일 뿐이다). */
const BACKGROUND_CARD_LIMIT = 7;

/**
 * 빌더 2단 레이아웃의 우측 프리뷰 패널. `BuilderPage`가 `renderPreview`
 * 콜백에 이 컴포넌트를 주입한다 — Shell은 이 위젯을 직접 import하지 않는다(주입 방식은
 * 그대로 유지).
 *
...
export function BuilderPreview<TFieldValues extends FieldValues>({
  ...
  // `kind`에 따라 다른 컴포넌트 타입을 반환하면(과거엔
  ...
  // 세션 지연 시작이 있어야 이 상시 마운트가 공짜다 — 첫 전송 전에는
...
function CardPreview<TFieldValues extends FieldValues>({
  ...
  // 필드 몇 개를 그대로 옮기는 값싼 호출이라(예전에 지운 `canPublish`의 `safeParse`와 다르다) 전체
  ...
  return (
    // `h-full`은 lg 미만에서 조상에 확정 높이가 없어(그
