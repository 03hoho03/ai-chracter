...
/** 창작 / 활동 / 계정 세 그룹 + 로그아웃.
 * `계정` 아래에 `고객센터` 그룹이 있다(처음엔 `공지사항` 한 항목이었고,
 * `문의하기`가 더해져 두 항목이 됐다).
 *
 ...
 * `고객센터`는 그 목록 밖의 새 그룹이라 그 그룹이 생기며 항목이 8→9개, 구분선이 4→5개로 늘었고, 같은
 * 그룹에 `문의하기`가 더해지며(구분선 추가 없음) 9→10개가 됐다. 같은 그룹에 `이용약관`·
 * `개인정보처리방침`이 더해지며(구분선 추가 없음) 10→12개가 됐고, `계정`
 * 그룹에 `대화 프로필`이 더해져(구분선 추가 없음) 12→13개가 됐다.
 * **세로 여유는 390×844가 아니라 640×844에서 잰다** — 이 드롭다운의 트리거는 `sm`(640px) 미만에서
 * `hidden`이고 그 폭에서는 모바일 드로어가 같은 목적지를 대신한다(390에는 잴 드롭다운이 없다). 13항목 실측:
 * 640×844에서 `[data-slot=dropdown-menu-content]` `scrollHeight` **591px** vs
 * `max-h` **794px** → 여유 203px, `data-clipped-below="false"`.
 */
  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        {/* `계정 메뉴`가 아니라 `메뉴`다 — 열두 항목 중 열하나가 목적지고 첫 그룹이 `창작`이라, 스크린리더가
      ...
      {/* 그룹 경계마다 구분선을 둔다 — **라벨만으로는 경계가 픽셀상 표시되지 않기 때문이다.**
          ...
          높이 비용은 그때 18px(350→368)이었고, `고객센터` 그룹(구분선 1 + 항목 1)이 얹히며
          61px(368→429)이 더 붙었다. 같은 그룹에 `문의하기`가 하나 더 얹히며(구분선 추가 없음)
          28px(429→457)이 다시 붙었다. 같은 그룹에 `이용약관`·`개인정보처리방침` 두 항목이 더
          얹히고(구분선 추가 없음), `대화 프로필`이 더해지며 높이가 다시 늘어 13항목 **591px**이다 —
          세로 여유는 640×844에서 **203px**(`max-h` 794px, 트리거가 `sm` 미만에서 숨으므로 390×844에는 잴
          드롭다운이 없다).
          ...
          **아래 여백**이라 글자가 16/16px 온전히 보였고, 넷일 때는 넘치는 26px이 `로그아웃` **행 자체**라
          글자가 **0/16px**로 한 픽셀도 안 보이되 행 박스는 6/28px 남아 있었다. `고객센터` 그룹으로 다섯이 된
          시점에는 넘치는 87px이었다 — 행 박스째로 화면 밖이라 `로그아웃`이 **0/28px**, 글자는
          0/17px로 그 6px 여백마저 사라졌다(`scrollTop: 0` 실측 A/B, 9항목 기준). `문의하기`로 항목이
          하나 더 늘어난 시점엔 넘치는 115px이었다(87+28) — 결함의 성격은 같다, `로그아웃` 행 전체가
          화면 밖이었다(`scrollTop: 0` 실측 A/B, 10항목 기준). **13항목인 지금은 `max-h` 340px에 591px이라
          넘치는 251px**이고 `scrollTop: 0`에서 `로그아웃` 행(top 606.6px)이 메뉴 bottom(390px) 아래로 통째로
          화면 밖이다.
          **여기 한때 "포인터·터치 사용자에게는 잘렸다는 신호가 아예 없다"고 적혀 있었는데, 그건 이제
          ...
          `::after` 스티키 그라디언트(`h-8`, `from-popover`)가 얹힌다. 13항목 상태의 844×390에서 실측하면
          `scrollTop: 0`에서 `data-clipped-below="true"`, 스크롤 끝(`scrollTop` 251px)에서 `로그아웃`이
          355.6~386.4px로 **30.8/30.8px 온전히** 보이고 clipped-below가 `"false"`로 걷힌다.
          잘렸다는 사실과 도달 경로가 둘 다 화면에 있다는 결론은 유지된다.
 ...
 * announce되고, 그래야 메뉴의 그룹 구조가 시각 사용자에게만 존재하지 않는다. */
