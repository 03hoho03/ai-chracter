 ...
 * `개인정보처리방침`을 더하며(구분선 추가 없음) 10→12개가 됐다. persona-goal-prompt.md UP-12가 `계정`
 * 그룹에 `대화 프로필`을 더해(구분선 추가 없음) 12→13개가 됐다.
 ...
 * (persona-progress.md S8 B5-1): 640×844에서 `[data-slot=dropdown-menu-content]` `scrollHeight` **591px** vs
  ...
  return (
    <DropdownMenu>
      ...
      {/* 그룹 경계마다 구분선을 둔다 — **라벨만으로는 경계가 픽셀상 표시되지 않기 때문이다.**
          ...
          얹으며(구분선 추가 없음), UP-12가 `대화 프로필`을 더하며 높이가 다시 늘어 13항목 **591px**이다 —
          세로 여유는 640×844에서 **203px**(`max-h` 794px, 트리거가 `sm` 미만에서 숨으므로 390×844에는 잴
          드롭다운이 없다 — persona-progress.md S8 B5-1).
          ...
          **아래 여백**이라 글자가 16/16px 온전히 보였고, 넷일 때는 넘치는 26px이 `로그아웃` **행 자체**라
          ...
          화면 밖이다(persona-progress.md S8 B5-1).
          **여기 한때 "포인터·터치 사용자에게는 잘렸다는 신호가 아예 없다"고 적혀 있었는데, 그건 이제
          ...
          355.6~386.4px로 **30.8/30.8px 온전히** 보이고 clipped-below가 `"false"`로 걷힌다(persona-progress.md
          S8 B5-1). 잘렸다는 사실과 도달 경로가 둘 다 화면에 있다는 결론은 유지된다.
