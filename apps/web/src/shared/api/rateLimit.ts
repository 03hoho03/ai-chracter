 ...
 * **수동 사본**이라 불일치를 잡는 CI 게이트가 없다. 값을 손으로 맞춰야 한다.
 *
 * ⚠️ `window`의 `"day"`는 BE가 더 이상 보내지 않는다(무료 일일분을
 ...
 * 말한다" 같은 화면 결정은 `entities/chat-room`의 `chatRateLimit.ts`가 진다. 그래서 위
