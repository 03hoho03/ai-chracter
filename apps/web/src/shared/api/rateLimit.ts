...
/** limit-goal-prompt.md RL-11 — BE `core/rate_limit_gate.py`가 내는 429 바디
 * `{"detail": {"code", "retryAfterSeconds", "window"}}`. `Retry-After` 헤더는 일부러 주지 않으므로
 * (CORS에서 못 읽는다) 재시도 시점의 유일한 출처가 이 detail이다.
 *
 * 단언 대신 스키마로 파싱하는 이유는 `client.ts`의 `errorEnvelopeSchema`와 같다 — 서버가 주는 외부
 * 데이터라 모양을 우리가 보장할 수 없다. `window`는 "어느 상한이냐"가 아니라 "어느 기능이냐"다:
 * 채팅 4경로는 minute/day, 이미지 생성은 image 하나이고 둘을 가르는 건 `code`다(BE 주석 RL-11/RL-12). */
const rateLimitDetailSchema = z.object({
  code: z.enum(["USER_LIMIT", "QUEUE_FULL"]),
  retryAfterSeconds: z.number(),
  window: z.enum(["minute", "day", "image"]),
});
