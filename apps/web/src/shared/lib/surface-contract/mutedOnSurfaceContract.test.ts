...
/**
 * 표면 표식이 있어도 `bg-muted`가 맞는 파일. 키는 glob 경로, 값은 **그 `bg-muted`가 어느 표면 위에
 * 있는지**를 적은 이유다. 항목이 더는 걸리지 않으면 테스트가 실패한다(낡은 예외를 남기지 않는다).
 */
const ALLOWLIST: Record<string, string> = {
  ...
  "../../../widgets/content-detail/ui/ContentDetailView.tsx":
    ...
    "위)만 bg-muted다. 표식은 modal 변형의 하단 바 bg-popover다.",
};

/** 표면 표식 — popover 표면을 여는 프리미티브 이름, 또는 표면 클래스 자체. */
const SURFACE_MARKER =
  /(?<![\w-])(?:DialogContent|AlertDialogContent|PopoverContent|SheetContent|DropdownMenuContent|SelectContent|bg-card|bg-popover)(?![\w-])/;
...
describe("card/popover 표면 위 bg-muted 금지", () => {
  it("검사 대상을 실제로 찾았다 — glob이 0건이면 아래 검사가 통째로 공회전한다", () => {
    ...
    expect(Object.keys(UI_SOURCES).length).toBeGreaterThanOrEqual(18);
  });
  ...
  it("허용 목록의 항목이 모두 아직 걸린다 — 고친 파일의 예외를 남기지 않는다", () => {
    const offenderPaths = new Set(OFFENDERS.map(({ path }) => path));
    const stale = Object.keys(ALLOWLIST).filter((path) => !offenderPaths.has(path));
