...
/** regenerate-ux-goal-prompt.md RU-2 — 재생성 클릭 즉시 옛 답변을 화면에서 지운다.
 * 반환값이 곧 "복원해야 할 것"이다(RU-3: 호출부는 이 값의 유무로 복원 여부를 판단한다).
 * regenerate-ux-techspec.md RT-3 — 마지막이 assistant 일 때만 지운다. 재생성 버튼은 마지막이
...
/** RU-4 — 실패 시 동기 롤백. RT-2: 같은 id가 이미 있으면 아무것도 하지 않는다(재조회가 먼저
