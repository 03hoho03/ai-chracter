 ...
 * 조용히 새 값이 채팅 배너로 새 나간다 — 허용 목록은 값이 늘어도 기본이 "안 받는다"다.
 *
...
export function getChatRateLimit(error: unknown): ChatRateLimit | undefined {
  ...
  // 🔴 `CLOVER_CONFIRM_REQUIRED`는 `window: "clover"`로 오지만
 ...
 *
 * `secondsLeft === 0`에서 `${secondsLeft}초 뒤 다시`를 그대로 쓰면 "0초 뒤 다시"가 남는다(실서버
 * 관측). 그 시점엔 버튼이 이미 `aria-disabled=false`로 풀려 클릭이 통하므로 문구도
