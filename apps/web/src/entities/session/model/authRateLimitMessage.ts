 ...
 * `getChatRateLimit`, `widgets/image-studio`의 `getImageRateLimit`과 같은 자리 — FSD-06, "이 슬라이스가
 ...
 * 직접 카운트다운한다(그 컴포넌트의 원래 용도, ED-18 ③). 그래서 이 함수가 두 코드를 같은 모양으로
 ...
 *
 * 은닉(ED-13) 관련: 아래 세 문구 모두 "요청 횟수는 호출자 자신의 행동이다"만 말하고 계정 존재를
 * 암시하지 않는다. `그 계정으로는 …`·`등록된 이메일로 …`·`이미 보낸 메일이 {N}통 있어요` 같은 단정형
 * 문구는 여기서 금지고, BE가 IP-429와 email-429를 `retry_after = ip_retry_after or email_retry_after`로
