...
/** `/my`·프로필 목록의 "더 보기". 홈·즐겨찾기의 sentinel 무한스크롤과 **일부러 다르다**:
 * 이 두 화면은 작가가 자기 작품을 관리하는 곳이라 목록 아래에 끝이 있어야 하고(스크롤이 계속 늘어나면
 * 마지막 항목에 도달했는지 알 수 없다), 명시적 버튼은 기획에서 확정한 사항이다.
 *
 * `variant="outline"`인 이유: DESIGN.md §Colors가 `primary` 솔리드를 이 시스템의 유일한 유채색 솔리드
 * 채움으로 못박았고 그 자리는 두 화면 모두 `작품 만들기`가 이미 갖고 있다 — 목록을 늘리는 건 그보다
 * 약한 보조 동작이다.
 *
 * **로딩 중 비활성은 `disabled`가 아니라 `aria-disabled`다.** `disabled`를 걸면 브라우저가
 ...
 * 남는다 — 이게 `disabled`와 갈리는 지점 전부다. 흐림 값만 base의 50이 아니라 **65**인데, 65는
 * **포커스가 닿는** 비활성에 따로 정한 값이다(50은 대비 미달로 기각됐다).
 *
 ...
 * "마지막 페이지에서 버튼이 사라진다"는 동작은 그대로 두고 **포커스만** 호출부가 준 타깃으로
