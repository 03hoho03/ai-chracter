...
/** 미발행 초안 삭제 확인. 발행작 완전 삭제는 정책상 존재하지 않으므로
      ...
      <DialogContent className="sm:max-w-sm">
        <DialogHeader>
          ...
          {/* 제목이 이미 말한 동작을 되풀이하지 않고 **결과만** 적는다(집안 문법 —
              ...
              바뀐다. 그래서 "목록에서 사라진다"는 사용자가 이미 가진 모델("여기선 지워지는 게
          ...
          >
            {/* `삭제` 두 글자면 옆의 `취소`와 46.2×32로 **픽셀 단위 쌍둥이**가 된다(실측) — 위험한
                ...
                **다만 이 처방은 `sm` 이상에서만 산다.** `DialogFooter`가 좁은 화면에서 세로로 접어
                ...
                된다(실측). 그 폭에서 둘을 가르는 건 채움 틴트와 글자, 그리고 순서뿐이다 — 지금
                그 순서는 `취소` **위**, 위험한 쪽 **아래**이고 **탭 순서와 일치한다**(전에는
