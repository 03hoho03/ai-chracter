        ...
        <DialogFooter>
          ...
          >
            {/* `삭제` 두 글자면 옆의 `취소`와 46.2×32로 **픽셀 단위 쌍둥이**가 된다(실측) — 위험한
                ...
                **다만 이 처방은 `sm` 이상에서만 산다.** `DialogFooter`가 좁은 화면에서
                `flex-col-reverse`로 접어 두 버튼을 전폭으로 만들기 때문에 390px에서 326×32,
                320px에서 256×32로 **다시 Δ0**이 된다(실측). 그 폭에서 둘을 가르는 건 채움 틴트와
                글자, 그리고 위험한 쪽이 `취소` **위**에 온다는 순서뿐이다. 그 순서는 **탭 순서와도
                어긋난다** — `flex-col-reverse`는 시각 순서만 뒤집고 DOM 순서는 그대로라 320px에서
                초기 포커스 `취소`(y453) → Tab → `편집한 내용 버리기`(y413) → Tab → `닫기`(y308)로
                아래에서 위로 올라간다(실측, WCAG 1.3.2). 좁은 화면의 이 배치는 shadcn
                `DialogFooter` 기본값이고 저장소의 확인 모달 6곳이 전부 같으므로 여기서만 바꾸지
                않는다 — 바꾸려면 프리미티브에서 여섯 곳을 한 번에. */}
