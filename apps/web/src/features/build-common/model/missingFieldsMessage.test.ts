...
describe("missingFieldsMessage", () => {
  ...
  it("라벨이 없는 키는 영문 원문 대신 일반 문구 하나로 접는다 (인덱스 경로)", () => {
...
describe("invalidFieldsMessage", () => {
  ...
  // `startingSetups.0.suggestedReplies`(추천 답변 배열 자체의 `.max(4)` 위반)를 예시로 쓴다 —
  ...
  // 라벨 맵에 없는 키라 지금도 fallback으로 접힌다(범위 밖 — 실측 확인됨).
  ...
  it("서버 400 경로와 같은 라벨 목록을 쓰고 문장만 갈린다", () => {
