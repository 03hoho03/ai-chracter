...
const RECONSENT_KINDS = ["terms", "privacy"] as const satisfies readonly LegalDocumentKind[];

/** 확정 결정 D-16 — 닫아도 "닫음" 상태를 저장하지 않는다. `dismissed`는 이 컴포넌트가 살아 있는
...
export function ReconsentModal() {
  ...
  const pendingKinds = RECONSENT_KINDS.filter(
    (kind) =>
  ...
  const open = !dismissed && pendingKinds.length > 0;
    ...
    <Dialog open={open} onOpenChange={handleOpenChange}>
      ...
      <DialogContent className="grid max-h-[calc(100dvh-2rem)] grid-rows-[auto_minmax(0,1fr)_auto] sm:max-w-lg">
        ...
        <div className="flex min-h-0 flex-col gap-6 overflow-y-auto border-t border-border pt-4">
          {pendingKinds.map((kind) => {
            const docQuery = docQueryByKind[kind];
            return (
              <section key={kind} className="flex flex-col gap-2">
                ...
                {docQuery.data && <Markdown content={docQuery.data.bodyMarkdown} />}
              </section>
            );
          })}
        </div>
