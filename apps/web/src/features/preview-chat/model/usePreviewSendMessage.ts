 ...
 * 소비 모양이지만, 미리보기는 재생성/수정/삭제가 없어(BE도 POST .../messages 하나뿐) send만
 * 남긴 단순화된 버전이다.
 *
 * `previewSessionId`는 훅 파라미터가 아니라 `send()` 호출마다 받는다 —
...
export function usePreviewSendMessage(
  ...
  async function send(
    ...
    // setStatus가 finally의 단일 호출이므로 429 정보도 그때까지 로컬 변수로 들고 있는다.
    ...
    try {
      for await (const event of openChatStream(
        ...
        if (event.type === "token") {
          ...
          // 여기서 즉시 setStatus({kind:"error"})를 부르지 않는다. isSending은 status.kind==="sending"의
          ...
          // 그래서 이 훅만 확정을 finally로 미뤄 옛 main처럼
